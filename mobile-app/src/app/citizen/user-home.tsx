...
export default function UserHomeScreen() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (loc) {
        ...
        const d = getDistance(
          { latitude: citizenData.latitude, longitude: citizenData.longitude },
          newCoords,
        );
        ...
        mapRef.current?.fitToCoordinates(
          [
            newCoords,
            {
              latitude: citizenData.latitude,
              longitude: citizenData.longitude,
            },
          ],
          {
  ...
  const handleOpenChat = () => {
    if (!activePickup) {
      Alert.alert(
        "No Active Pickup",
        "You don't have a pickup scheduled for today.",
      );
      return;
    }
    ...
    if (!isDriverOnline) {
      Alert.alert(
        "Driver Offline",
        "The assigned driver is not currently online.",
      );
      return;
    }
      ...
      <View style={styles.header}>
        <View style={styles.headerTextContainer}>
          <Text style={styles.welcomeText}>
            Welcome, {citizenData?.full_name || "User"}
          </Text>
              ...
              <>
                <Marker coordinate={driverLocation} title="Collector">
                  <Ionicons
                    name="bus"
                    size={32}
                    color={isDriverOnline ? "#10B981" : "#9CA3AF"}
                  />
                </Marker>
